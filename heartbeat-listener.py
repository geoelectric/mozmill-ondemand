...
def got_message(data, message):
    ...
    output("Got heartbeat; sent: %s, received: %s" % (sent, received))
    message.ack()

def main():
    # unique applabel
    label = 'mozmill.heartbeat-%s' % (uuid.uuid1().hex)
    pulse = consumers.PulseTestConsumer(applabel=label)
    output("Registered with pulse server as %s" % (label))
    ...
    output("Listening for heartbeats...")
    pulse.listen()
